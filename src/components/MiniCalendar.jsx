  ...
  return (
    <div className="p-3 border-t border-subtle mt-auto">
      <div className="flex items-center justify-between mb-2">
        <button onClick={prev} className="text-faint hover:text-primary p-0.5 rounded transition-colors"><ChevronLeft size={14} /></button>
        <span className="text-[11px] text-muted font-medium">{label}</span>
        <button onClick={next} className="text-faint hover:text-primary p-0.5 rounded transition-colors"><ChevronRight size={14} /></button>
      </div>
      <div className="grid grid-cols-7 gap-0.5">
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => onSelectDate(ds)} className={cn(
              "text-[10px] text-center py-1 rounded transition-all",
              isToday && "bg-[var(--accent)] text-[var(--bg-primary)] font-bold",
              isSel   && "bg-elevated text-primary",
              !isToday && !isSel && "text-muted hover:bg-surface hover:text-primary"
            )}>
              {d}
              {hasTask && !isToday && <span className="block w-1 h-1 bg-teal-400 rounded-full mx-auto mt-0.5" />}
            </button>
          )
        })}
      </div>
