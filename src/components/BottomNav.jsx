import { CalendarDays, History, Menu, Search } from "lucide-react"
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-30 md:hidden border-t border-subtle bg-card safe-pb">
      <div className="flex items-center h-16">
        ...
        <button
          onClick={onMenuOpen}
          className="flex-1 flex flex-col items-center justify-center gap-1 h-full text-faint hover:text-primary transition-colors"
        >
          <Menu size={20} />
        ...
        {tabs.map(tab => (
          ...
          >
            ...
            {view === tab.id && (
              <span className="absolute top-0 inset-x-4 h-0.5 bg-teal-400 rounded-full" />
            )}
          </button>
        ))}
