  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg">
        ...
        {!tasks.length && <p className="text-xs text-faint text-center py-8">no tasks logged for this day.</p>}

        {done.length > 0 && (
          <div className="mb-4">
            <p className="text-[9px] text-green-400 uppercase tracking-widest font-medium mb-2">completed ({done.length})</p>
            {done.map(t => (
              <div key={t.id} className="flex items-start gap-2.5 py-1.5 border-b border-subtle last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] text-primary leading-snug">{t.title}</p>
        ...
        {remaining.length > 0 && (
          <div className="mb-4">
            <p className="text-[9px] text-amber-400 uppercase tracking-widest font-medium mb-2">open / in progress ({remaining.length})</p>
            {remaining.map(t => (
              <div key={t.id} className="flex items-start gap-2.5 py-1.5 border-b border-subtle last:border-0">
